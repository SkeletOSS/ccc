...
check_static_begin(
    check_range,
    Array_tree_map const *const map,
...
check_static_begin(array_tree_map_test_valid_range) {
    ...
    check(
        check_range(
            &s,
            array_tree_map_equal_range_wrap(&s, &(int){6}, &(int){44}),
            8,
    ...
    check(
        check_range_reverse(
            &s,
            array_tree_map_equal_range_reverse_wrap(
                &s, &(int){119}, &(int){84}
            ),
            8,
...
check_static_begin(array_tree_map_test_valid_range_equals) {
    ...
    check(
        check_range(
            &s,
            array_tree_map_equal_range_wrap(&s, &(int){10}, &(int){40}),
            8,
            (int[8]){10, 15, 20, 25, 30, 35, 40, 45}
        ),
        CHECK_PASS
...
check_static_begin(array_tree_map_test_invalid_range) {
    ...
    check(
        check_range(
            &s,
            array_tree_map_equal_range_wrap(&s, &(int){95}, &(int){999}),
            6,
...
check_static_begin(array_tree_map_test_empty_range) {
    ...
    check(
        ((struct Val *)array_tree_map_at(&s, range_begin(&forward_range)))->id,
        0
    );
    check(
        ((struct Val *)array_tree_map_at(&s, range_end(&forward_range)))->id, 0
    );
    ...
    check(
        ((struct Val *)array_tree_map_at(&s, range_reverse_begin(&rev_range)))
            ->id,
        (num_nodes * step) - step
    );
    check(
        ((struct Val *)array_tree_map_at(&s, range_reverse_end(&rev_range)))
            ->id,
        (num_nodes * step) - step
    );
    check_end();
...
main(void) {
    return check_run(
        array_tree_map_test_forward_iterator(),
