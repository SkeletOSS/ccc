...
check_static_begin(array_adaptive_map_test_remove_key_value) {
    ...
    check(count(&array_adaptive_map).count, 1);
    struct Val old = {.id = -1};
    hndl = CCC_remove_key_value(&array_adaptive_map, &old);
    check(validate(&array_adaptive_map), true);
    check(occupied(&hndl), true);
    check(count(&array_adaptive_map).count, 0);
    check(old.val, -1);
...
check_static_begin(array_adaptive_map_test_try_insert_with) {
    ...
    int size = 30;
    CCC_Handle *hndl = array_adaptive_map_try_insert_with(
        &array_adaptive_map, -1, &(CCC_Allocator){}, val(-1)
    );
    check(validate(&array_adaptive_map), true);
    check(occupied(hndl), false);
    check(count(&array_adaptive_map).count, 1);
    hndl = array_adaptive_map_try_insert_with(
        &array_adaptive_map, -1, &(CCC_Allocator){}, val(-1)
    );
    check(validate(&array_adaptive_map), true);
    check(occupied(hndl), true);
    ...
    i += (size / 2);
    hndl = array_adaptive_map_try_insert_with(
        &array_adaptive_map, i, &(CCC_Allocator){}, val(i)
    );
    check(validate(&array_adaptive_map), true);
    check(occupied(hndl), false);
    check(count(&array_adaptive_map).count, i + 2);
...
check_static_begin(array_adaptive_map_test_insert_or_assign_with) {
    ...
    hndl = array_adaptive_map_insert_or_assign_with(
        &array_adaptive_map, -1, &(CCC_Allocator){}, val(-2)
    );
    check(validate(&array_adaptive_map), true);
    check(occupied(hndl), true);
    check(count(&array_adaptive_map).count, 1);
    ...
    check(count(&array_adaptive_map).count, i + 2);
    hndl = array_adaptive_map_insert_or_assign_with(
        &array_adaptive_map, i, &(CCC_Allocator){}, val(i + 1)
    );
    check(occupied(hndl), true);
    check(count(&array_adaptive_map).count, i + 2);
    v = array_adaptive_map_at(&array_adaptive_map, unwrap(hndl));
...
check_static_begin(array_adaptive_map_test_array_and_modify) {
    ...
    CCC_Array_adaptive_map_handle *hndl
        = array_adaptive_map_handle_wrap(&array_adaptive_map, &(int){-1});
    check(validate(&array_adaptive_map), true);
    check(occupied(hndl), false);
    check(count(&array_adaptive_map).count, 0);
    ...
    check(validate(&array_adaptive_map), true);
    hndl = array_adaptive_map_handle_wrap(&array_adaptive_map, &(int){-1});
    check(occupied(hndl), true);
    check(count(&array_adaptive_map).count, 1);
    struct Val *v = array_adaptive_map_at(&array_adaptive_map, unwrap(hndl));
    check(v != NULL, true);
    ...
    i += (size / 2);
    hndl = array_adaptive_map_handle_wrap(&array_adaptive_map, &i);
    check(occupied(hndl), false);
    check(count(&array_adaptive_map).count, i + 1);
    (void)array_adaptive_map_insert_or_assign_with(
        &array_adaptive_map, i, &(CCC_Allocator){}, val(i)
...
check_static_begin(array_adaptive_map_test_array_and_modify_with) {
    ...
    CCC_Array_adaptive_map_handle *hndl
        = array_adaptive_map_handle_wrap(&array_adaptive_map, &(int){-1});
    hndl = array_adaptive_map_and_modify_with(hndl, struct Val * e, {
        e->val++;
    });
    check(count(&array_adaptive_map).count, 0);
    check(occupied(hndl), false);
...
check_static_begin(array_adaptive_map_test_remove_handle) {
    ...
    check(v->val, -1);
    check(count(&array_adaptive_map).count, 1);
    CCC_Handle *e = array_adaptive_map_remove_handle_wrap(
        array_adaptive_map_handle_wrap(&array_adaptive_map, &(int){-1})
    );
    check(validate(&array_adaptive_map), true);
    check(occupied(e), true);
    v = array_adaptive_map_at(&array_adaptive_map, unwrap(e));
    check(v != NULL, true);
    check(v->id, -1);
    check(v->val, -1);
    check(count(&array_adaptive_map).count, 0);
    ...
    check(v->val, i);
    check(count(&array_adaptive_map).count, i + 1);
    e = array_adaptive_map_remove_handle_wrap(
        array_adaptive_map_handle_wrap(&array_adaptive_map, &i)
    );
    check(validate(&array_adaptive_map), true);
    check(occupied(e), true);
...
main(void) {
    return check_run(
        ...
        array_adaptive_map_test_insert_handle_with(),
