...
check_static_begin(array_adaptive_map_test_validate) {
    ...
    hndl = swap_handle(&array_adaptive_map, &(struct Val){.id = -1, .val = -1});
    check(validate(&array_adaptive_map), true);
...
check_static_begin(array_adaptive_map_test_insert) {
    ...
    i += (size / 2);
    hndl = swap_handle(&array_adaptive_map, &(struct Val){.id = i, .val = i});
    check(validate(&array_adaptive_map), true);
    check(occupied(&hndl), false);
...
check_static_begin(array_adaptive_map_test_try_insert) {
    ...
    check(occupied(&hndl), false);
    check(count(&array_adaptive_map).count, 1);
    hndl = try_insert(&array_adaptive_map, &(struct Val){.id = -1, .val = -1});
    check(validate(&array_adaptive_map), true);
    check(occupied(&hndl), true);
    check(count(&array_adaptive_map).count, 1);
    struct Val *v = array_adaptive_map_at(&array_adaptive_map, unwrap(&hndl));
    ...
    i += (size / 2);
    hndl = try_insert(&array_adaptive_map, &(struct Val){.id = i, .val = i});
    check(validate(&array_adaptive_map), true);
    check(occupied(&hndl), false);
    check(count(&array_adaptive_map).count, i + 2);
    hndl = try_insert(&array_adaptive_map, &(struct Val){.id = i, .val = i});
...
check_static_begin(array_adaptive_map_test_try_insert_with) {
    ...
    check(occupied(hndl), false);
    check(count(&array_adaptive_map).count, 1);
    hndl = array_adaptive_map_try_insert_with(&array_adaptive_map, -1, val(-1));
    check(validate(&array_adaptive_map), true);
    check(occupied(hndl), true);
    check(count(&array_adaptive_map).count, 1);
    struct Val *v = array_adaptive_map_at(&array_adaptive_map, unwrap(hndl));
    ...
    i += (size / 2);
    hndl = array_adaptive_map_try_insert_with(&array_adaptive_map, i, val(i));
    check(validate(&array_adaptive_map), true);
    check(occupied(hndl), false);
    check(count(&array_adaptive_map).count, i + 2);
    hndl = array_adaptive_map_try_insert_with(&array_adaptive_map, i, val(i));
...
check_static_begin(array_adaptive_map_test_remove_handle) {
    ...
    check(count(&array_adaptive_map).count, 1);
    CCC_Handle *e
        = remove_handle_wrap(handle_wrap(&array_adaptive_map, &(int){-1}));
    check(validate(&array_adaptive_map), true);
    ...
    check(v->val, i);
    check(count(&array_adaptive_map).count, i + 1);
    e = remove_handle_wrap(handle_wrap(&array_adaptive_map, &i));
    check(validate(&array_adaptive_map), true);
    check(occupied(e), true);
    v = array_adaptive_map_at(&array_adaptive_map, unwrap(e));
    check(v != NULL, true);
