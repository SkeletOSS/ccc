...
check_static_begin(adaptive_map_test_remove_key_value) {
    ...
    CCC_Entry ent = CCC_remove_key_value(
        &om, &(struct Val){.key = -1, .val = -1}.elem, &allocator);
    check(validate(&om), true);
    check(occupied(&ent), false);
    check(unwrap(&ent), NULL);
...
check_static_begin(adaptive_map_test_try_insert_with) {
    ...
    check(unwrap(ent) != NULL, true);
    check(count(&om).count, 1);
    ent = adaptive_map_try_insert_with(&om, -1, &allocator, val(-1));
    check(validate(&om), true);
    check(occupied(ent), true);
    check(count(&om).count, 1);
    struct Val *v = unwrap(ent);
...
check_static_begin(adaptive_map_test_insert_or_assign_with) {
    ...
    int size = 30;
    CCC_Entry *ent
        = adaptive_map_insert_or_assign_with(&om, -1, &allocator, val(-1));
    check(validate(&om), true);
    check(occupied(ent), false);
    check(unwrap(ent) != NULL, true);
    check(count(&om).count, 1);
    ent = adaptive_map_insert_or_assign_with(&om, -1, &allocator, val(-2));
    check(validate(&om), true);
    check(occupied(ent), true);
    check(count(&om).count, 1);
    struct Val *v = unwrap(ent);
    ...
    check(unwrap(ent) != NULL, true);
    check(count(&om).count, i + 2);
    ent = adaptive_map_insert_or_assign_with(&om, i, &allocator, val(i + 1));
    check(occupied(ent), true);
    check(count(&om).count, i + 2);
    v = unwrap(ent);
    check(v != NULL, true);
...
check_static_begin(adaptive_map_test_entry_and_modify) {
    ...
    check(unwrap(&ent) == NULL, true);
    check(count(&om).count, 0);
    (void)adaptive_map_insert_or_assign_with(&om, -1, &allocator, val(-1));
    check(validate(&om), true);
    ent = entry(&om, &(int){-1});
    check(occupied(&ent), true);
    check(count(&om).count, 1);
...
check_static_begin(adaptive_map_test_entry_and_context_modify) {
    ...
    int context = 1;
    CCC_Adaptive_map_entry ent = entry(&om, &(int){-1});
    ent = *and_modify(&ent, &(CCC_Modifier){
                                .modify = pluscontext,
                                .context = &context,
                            });
    check(occupied(&ent), false);
...
check_static_begin(adaptive_map_test_entry_and_modify_with) {
    ...
    (void)adaptive_map_insert_or_assign_with(&om, -1, &allocator, val(-1));
    check(validate(&om), true);
    ent = adaptive_map_entry_wrap(&om, &(int){-1});
    struct Val *v = unwrap(ent);
    check(v != NULL, true);
    check(v->val, -1);
    check(v->key, -1);
...
check_static_begin(adaptive_map_test_or_insert) {
    ...
    int size = 30;
    struct Val *v
        = or_insert(adaptive_map_entry_wrap(&om, &(int){-1}),
                    &(struct Val){.key = -1, .val = -1}.elem, &allocator);
    check(validate(&om), true);
    check(v != NULL, true);
    check(v->key, -1);
    check(v->val, -1);
    check(count(&om).count, 1);
    v = or_insert(adaptive_map_entry_wrap(&om, &(int){-1}),
                  &(struct Val){.key = -1, .val = -2}.elem, &allocator);
    check(v != NULL, true);
    check(v->key, -1);
    check(v->val, -1);
    check(count(&om).count, 1);
    ...
    i += (size / 2);
    v = or_insert(adaptive_map_entry_wrap(&om, &i),
                  &(struct Val){.key = i, .val = i}.elem, &allocator);
    check(validate(&om), true);
    check(v != NULL, true);
    check(v->key, i);
    check(v->val, i);
    check(count(&om).count, i + 2);
    v = or_insert(adaptive_map_entry_wrap(&om, &i),
                  &(struct Val){.key = i, .val = i + 1}.elem, &allocator);
    check(v != NULL, true);
    check(v->key, i);
    check(v->val, i);
    check(count(&om).count, i + 2);
...
check_static_begin(adaptive_map_test_or_insert_with) {
    ...
    struct Val *v = adaptive_map_or_insert_with(
        adaptive_map_entry_wrap(&om, &(int){-1}), &allocator, idval(-1, -1));
    check(validate(&om), true);
    check(v != NULL, true);
    check(v->key, -1);
    check(v->val, -1);
    check(count(&om).count, 1);
    v = adaptive_map_or_insert_with(adaptive_map_entry_wrap(&om, &(int){-1}),
                                    &allocator, idval(-1, -2));
    check(v != NULL, true);
    check(v->key, -1);
    check(v->val, -1);
    ...
    i += (size / 2);
    v = adaptive_map_or_insert_with(adaptive_map_entry_wrap(&om, &i),
                                    &allocator, idval(i, i));
    check(validate(&om), true);
    check(v != NULL, true);
    check(v->key, i);
...
check_static_begin(adaptive_map_test_insert_entry) {
    ...
    int size = 30;
    struct Val *v
        = insert_entry(adaptive_map_entry_wrap(&om, &(int){-1}),
                       &(struct Val){.key = -1, .val = -1}.elem, &allocator);
    check(validate(&om), true);
    check(v != NULL, true);
    check(v->key, -1);
    check(v->val, -1);
    check(count(&om).count, 1);
    v = insert_entry(adaptive_map_entry_wrap(&om, &(int){-1}),
                     &(struct Val){.key = -1, .val = -2}.elem, &allocator);
    check(v != NULL, true);
    check(v->key, -1);
    check(v->val, -2);
    ...
    i += (size / 2);
    v = insert_entry(adaptive_map_entry_wrap(&om, &i),
                     &(struct Val){.key = i, .val = i}.elem, &allocator);
    check(validate(&om), true);
    check(v != NULL, true);
    check(v->key, i);
...
check_static_begin(adaptive_map_test_insert_entry_with) {
    ...
    check(v->val, -1);
    check(count(&om).count, 1);
    v = adaptive_map_insert_entry_with(adaptive_map_entry_wrap(&om, &(int){-1}),
                                       &allocator, idval(-1, -2));
    check(v != NULL, true);
    check(v->key, -1);
    check(v->val, -2);
    ...
    i += (size / 2);
    v = adaptive_map_insert_entry_with(adaptive_map_entry_wrap(&om, &i),
                                       &allocator, idval(i, i));
    check(validate(&om), true);
    check(v != NULL, true);
    check(v->key, i);
...
check_static_begin(adaptive_map_test_remove_entry) {
    ...
    check(v->val, -1);
    check(count(&om).count, 1);
    CCC_Entry *e = adaptive_map_remove_entry_wrap(
        adaptive_map_entry_wrap(&om, &(int){-1}), &allocator);
    check(validate(&om), true);
    ...
    check(v->val, i);
    check(count(&om).count, i + 1);
    e = adaptive_map_remove_entry_wrap(adaptive_map_entry_wrap(&om, &i),
                                       &allocator);
    check(validate(&om), true);
    check(occupied(e), true);
    check(count(&om).count, i);
