...
static size_t bubble_up(CCC_Buffer *buffer, CCC_Order order,
                        CCC_Type_comparator *compare, void *temp, size_t index);
...
CCC_flat_priority_queue_heapsort(CCC_Buffer *const buffer, CCC_Order order,
    ...
    if (buffer->count > 1) {
        ...
        while (--end) {
            CCC_buffer_swap(buffer, temp, 0, end);
            (void)bubble_down(buffer, order, compare, temp, 0, end);
        }
    }
    return CCC_RESULT_OK;
}
...
CCC_flat_priority_queue_push(CCC_Flat_priority_queue *const priority_queue,
    ...
    assert(temp);
    size_t const i = bubble_up(&priority_queue->buffer, priority_queue->order,
                               priority_queue->compare, temp,
...
CCC_private_flat_priority_queue_bubble_up(
    ...
    return bubble_up(&priority_queue->buffer, priority_queue->order,
                     priority_queue->compare, temp, index);
}
...
heapify(CCC_Buffer *const buffer, CCC_Order const order,
    ...
    while (i--) {
        (void)bubble_down(buffer, order, compare, temp, i, buffer->count);
    }
}
...
bubble_up(CCC_Buffer *const buffer, CCC_Order const order,
          CCC_Type_comparator *const compare, void *const temp, size_t index) {
    for (size_t parent = (index - 1) / 2; index;
        ...
        if (!wins(order, compare, buffer->context, this_pointer,
                  parent_pointer)) {
            return index;
        }
...
bubble_down(CCC_Buffer *const buffer, CCC_Order const order,
    ...
    for (size_t next = 0, left = (index * 2) + 1, right = left + 1;
        ...
        if (right < count) {
            ...
            if (wins(order, compare, buffer->context, right_pointer,
                     left_pointer)) {
                next = right;
            }
        }
        ...
        if (!wins(order, compare, buffer->context, next_pointer,
                  this_pointer)) {
            return index;
        }
...
order(struct CCC_Flat_priority_queue const *const priority_queue, size_t left,
    ...
    return priority_queue->compare((CCC_Type_comparator_context){
        .type_left = CCC_buffer_at(&priority_queue->buffer, left),
        .type_right = CCC_buffer_at(&priority_queue->buffer, right),
