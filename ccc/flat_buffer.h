...
@endcond */
/** @file
@brief The Flat_buffer Interface

Flat_buffer usage is similar to a C++ vector, with more flexible functions
...
in this library that provide more specialized operations. A Flat_buffer does not
require the user accommodate any intrusive elements.

A Flat_buffer offers a more flexible interface than a standard C++ vector. There
are functions that assume elements are stored contiguously from `[0, N)` where
`N` is the count of elements. However, there are also functions that let the
user access any Flat_buffer slot that is within the bounds of Flat_buffer
capacity. This requires the user pay closer attention to Flat_buffer usage but
ultimately allows a wider variety of abstractions on top of the buffer.
...
A Flat_buffer function that accepts an allocator will re-size as required when a
new element is inserted in a contiguous fashion if an allocator is provided.
...
#ifndef CCC_FLAT_BUFFER_H
#define CCC_FLAT_BUFFER_H
...
A Flat_buffer may be initialized on the stack, heap, or data segment at compile
time or runtime. */
...
@return the initialized buffer. Directly assign to Flat_buffer on the right hand
side of the equality operator.

Initialization of a Flat_buffer can occur at compile time or run time.

```
static CCC_Flat_buffer stack = CCC_flat_buffer_default(int);
```

The buffer will be empty with a count and capacity of 0. */
#define CCC_flat_buffer_default(type_name)                                     \
    CCC_private_flat_buffer_default(type_name)

/** @brief Initialize a contiguous Flat_buffer of user a specified type,
capacity, and optional starting size.
@param[in] type_name the name of the user type in the buffer.
@param[in] capacity the capacity of memory at data_pointer.
@param[in] count optional starting size of the Flat_buffer <= capacity.
@param[in] data_pointer the pointer to existing memory or NULL.
@return the initialized buffer. Directly assign to Flat_buffer on the right hand
side of the equality operator.

Initialization of a Flat_buffer can occur at compile time or run time depending
...
/** @brief Initialize a Flat_buffer from a compound literal array initializer.
@param[in] allocator a pointer to
CCC_Allocator.
@param[in] optional_capacity optionally specify the capacity of the Flat_buffer
if different from the size of the compound literal array initializer. If the
...
@return the initialized buffer. Directly assign to Flat_buffer on the right hand
side of the equality operator (e.g. CCC_Flat_buffer b =
...
Initialize a dynamic Flat_buffer with a compound literal array with capacity and
context
...
CCC_flat_buffer_for() macro. */
#define CCC_flat_buffer_from(                                                  \
...
/** @brief Initialize a Flat_buffer with a capacity.
@param[in] type_name any user or language standard type name.
@param[in] allocator a pointer to
CCC_Allocator.
@param[in] capacity the capacity of the Flat_buffer to reserve.
@return the initialized buffer. Directly assign to Flat_buffer on the right hand
side of the equality operator (e.g. CCC_Flat_buffer b =
...
/** @brief Initialize a contiguous Flat_buffer of user a specified type of fixed
capacity with no allocation permission or context.
@param[in] count starting count of the Flat_buffer <= capacity of input literal.
@param[in] compound_literal_array the compound literal array of types.
@return the initialized buffer. Directly assign to Flat_buffer on the right hand
side of the equality operator
(e.g. CCC_Flat_buffer b = CCC_flat_buffer_with_storage(...);).

Initialization of a Flat_buffer can occur at compile time or run time but always
...
status is returned. */
[[nodiscard]] CCC_Result CCC_flat_buffer_reserve(
    CCC_Flat_buffer *buffer, size_t to_add, CCC_Allocator const *allocator
);
...
[[nodiscard]] CCC_Result CCC_flat_buffer_copy(
    CCC_Flat_buffer *destination,
...
These functions assume contiguity of elements in the Flat_buffer and increase or
decrease size accordingly. */
/**@{*/

/** @brief allocates the Flat_buffer to the specified size according to the user
...
[[nodiscard]] CCC_Result CCC_flat_buffer_allocate(
    CCC_Flat_buffer *buffer, size_t capacity, CCC_Allocator const *allocator
);

/** @brief allocates a new slot from the Flat_buffer at the end of the
contiguous array. A slot is equivalent to one of the element type specified when
the Flat_buffer is initialized.
@param[in] buffer a pointer to the buffer.
@param[in] allocator the allocation context defined by the user.
@return a pointer to the newly allocated memory or NULL if no Flat_buffer is
provided or the Flat_buffer is unable to allocate more memory.
@note this function modifies the size of the container.

A Flat_buffer can be used as the backing for more complex data structures.
Requesting new space from a Flat_buffer as an allocator can be helpful for these
higher level organizations. */
[[nodiscard]] void *CCC_flat_buffer_allocate_back(
    CCC_Flat_buffer *buffer, CCC_Allocator const *allocator
);
...
@param[in] allocator the allocation context defined by the user.
@return the pointer to the newly pushed element or NULL if no Flat_buffer exists
or resizing has failed due to memory exhuastion or no allocation allowed.
@note this function modifies the size of the container.

The data is copied into the Flat_buffer at the final slot if there is remaining
...
exhausted. */
[[nodiscard]] void *CCC_flat_buffer_push_back(
    CCC_Flat_buffer *buffer, void const *data, CCC_Allocator const *allocator
);
...
@param[in] flat_buffer_pointer a pointer to the buffer.
...
if the Flat_buffer fails to allocate a slot at the back of the buffer. This may
occur if resizing fails or is prohibited. */
#define CCC_flat_buffer_emplace_back(                                          \
...
/** @brief insert data at slot index according to size of the Flat_buffer
maintaining contiguous storage of elements between 0 and size.
@param[in] buffer the pointer to the buffer.
@param[in] index the index at which to insert data.
@param[in] data the data copied into the Flat_buffer at index index of the same
size
@param[in] allocator the allocation context defined by the user.
as elements stored in the buffer.
@return the pointer to the inserted element or NULL if bad input is provided,
the Flat_buffer is full and no resizing is allowed, or resizing fails when
resizing is allowed.
@note this function modifies the size of the container.

Note that this function assumes elements must be maintained contiguously
according to size of the Flat_buffer meaning a bulk move of elements sliding
down to accommodate index will occur. */
[[nodiscard]] void *CCC_flat_buffer_insert(
...
/** @brief pop the back element from the Flat_buffer according to size.
@param[in] buffer the pointer to the buffer.
...
/** @brief pop count elements from the back of the Flat_buffer according to
size.
@param[in] buffer the pointer to the buffer.
@param[in] count the number of elements to pop.
@return the result of the attempted pop. CCC_RESULT_OK if the Flat_buffer exists
and n is within the bounds of size. If the Flat_buffer does not exist an input
error is returned. If count is greater than the size of the Flat_buffer size is
set to zero and input error is returned.
@note this function modifies the size of the container. */
CCC_Result CCC_flat_buffer_pop_back_n(CCC_Flat_buffer *buffer, size_t count);

/** @brief erase element at slot index according to size of the Flat_buffer
maintaining contiguous storage of elements between 0 and size.
@param[in] buffer the pointer to the buffer.
@param[in] index the index of the element to be erased.
@return the result, CCC_RESULT_OK if the input is valid. If no Flat_buffer
exists or i is out of range of size then an input error is returned.
@note this function modifies the size of the container.
...
These functions interact with slots in the Flat_buffer directly and do not
modify the size of the buffer. These are best used for custom container types
...
Note that as long as the index is valid within the capacity of the Flat_buffer a
...
size of the buffer. */
[[nodiscard]] void *
CCC_flat_buffer_at(CCC_Flat_buffer const *buffer, size_t index);

/** @brief Access en element at the specified index as the stored type.
@param[in] flat_buffer_pointer the pointer to the buffer.
...
data. It is up to the user to ensure the index provided is within the current
size of the buffer. */
#define CCC_flat_buffer_as(flat_buffer_pointer, type_name, index)              \
...
[[nodiscard]] CCC_Count
CCC_flat_buffer_index(CCC_Flat_buffer const *buffer, void const *slot);

/** @brief return the final element in the Flat_buffer according the current
size.
@param[in] buffer the pointer to the buffer.
@return the pointer the final element in the Flat_buffer according to the
current size or NULL if the Flat_buffer does not exist or is empty. */
[[nodiscard]] void *CCC_flat_buffer_back(CCC_Flat_buffer const *buffer);
...
@return the pointer the final element in the Flat_buffer according to the
current size or NULL if the Flat_buffer does not exist or is empty. */
#define CCC_flat_buffer_back_as(flat_buffer_pointer, type_name)                \
    ((type_name *)CCC_flat_buffer_back(flat_buffer_pointer))

/** @brief return the first element in the Flat_buffer at index 0.
@param[in] buffer the pointer to the buffer.
@return the pointer to the front element or NULL if the Flat_buffer does not
exist or is empty. */
[[nodiscard]] void *CCC_flat_buffer_front(CCC_Flat_buffer const *buffer);
...
@return the pointer to the front element or NULL if the Flat_buffer does not
exist or is empty. */
#define CCC_flat_buffer_front_as(flat_buffer_pointer, type_name)               \
...
void *CCC_flat_buffer_move(
    CCC_Flat_buffer const *buffer, size_t destination, size_t source
);

/** @brief write data to Flat_buffer at slot at index index according to
capacity.
@param[in] buffer the pointer to the buffer.
@param[in] index the index within bounds of capacity of the buffer.
@param[in] data the data that will be written to slot at i.
@return the result of the write, CCC_RESULT_OK if success. If no Flat_buffer or
data exists input error is returned. If index is outside of the range of
capacity input error is returned.
@note this function does NOT modify the size of the container.
...
of the Flat_buffer if such behavior is desired. No elements are moved to be
preserved meaning any data at index is overwritten. */
CCC_Result CCC_flat_buffer_write(
    CCC_Flat_buffer const *buffer, size_t index, void const *data
);

/** @brief Writes a user provided compound literal directly to a Flat_buffer
slot.
@param[in] flat_buffer_pointer a pointer to the buffer.
...
Any function calls that set fields of the compound literal will not be evaluated
if the provided index is out of range of the Flat_buffer capacity. */
#define CCC_flat_buffer_emplace(                                               \
...
bufer.
@param[in] buffer the pointer to the buffer.
@param[in] temp the pointer to the temporary Flat_buffer of the same size as an
...
CCC_Result CCC_flat_buffer_swap(
    CCC_Flat_buffer const *buffer, void *temp, size_t index, size_t swap_index
);
...
/** @brief obtain the base address of the Flat_buffer in preparation for
iteration.
@param[in] buffer the pointer to the buffer.
@return the base address of the buffer. This will be equivalent to the buffer
end iterator if the Flat_buffer size is 0. NULL is returned if a NULL argument
is provided or the Flat_buffer has not yet been allocated. */
[[nodiscard]] void *CCC_flat_buffer_begin(CCC_Flat_buffer const *buffer);

/** @brief advance the iterator to the next slot in the Flat_buffer according to
size.
@param[in] buffer the pointer to the buffer.
@param[in] iterator the pointer to the current slot of the buffer.
@return the next iterator position according to size. */
[[nodiscard]] void *
CCC_flat_buffer_next(CCC_Flat_buffer const *buffer, void const *iterator);

/** @brief return the end position of the Flat_buffer according to size.
@param[in] buffer the pointer to the buffer.
@return the address of the end position. It is undefined to access this
position for any reason. NULL is returned if NULL is provided or Flat_buffer has
not yet been allocated.

Note that end is determined by the size of the Flat_buffer dynamically. */
[[nodiscard]] void *CCC_flat_buffer_end(CCC_Flat_buffer const *buffer);

/** @brief obtain the address of the last element in the Flat_buffer in
preparation for iteration according to size.
@param[in] buffer the pointer to the buffer.
@return the address of the last element buffer. This will be equivalent to the
Flat_buffer reverse_end iterator if the Flat_buffer size is 0. NULL is returned
if a NULL argument is provided or the Flat_buffer has not yet been allocated. */
[[nodiscard]] void *
CCC_flat_buffer_reverse_begin(CCC_Flat_buffer const *buffer);

/** @brief advance the iterator to the next slot in the Flat_buffer according to
size and in reverse order.
@param[in] buffer the pointer to the buffer.
@param[in] iterator the pointer to the current slot of the buffer.
@return the next iterator position according to size and in reverse order. NULL
is returned if bad input is provided or the Flat_buffer has not been allocated.
*/
[[nodiscard]] void *CCC_flat_buffer_reverse_next(
    CCC_Flat_buffer const *buffer, void const *iterator
);
...
position for any reason. NULL is returned if NULL is provided or Flat_buffer has
not yet been allocated. */
[[nodiscard]] void *CCC_flat_buffer_reverse_end(CCC_Flat_buffer const *buffer);
...
@param[in] buffer the pointer to the buffer.
@param[in] count the quantity to add to the current Flat_buffer size.
@return the result of resizing. CCC_RESULT_OK if no errors occur or an error
indicating bad input has been provided.

If count would exceed the current capacity of the Flat_buffer the size is set to
...
@param[in] buffer the pointer to the buffer.
@param[in] count the quantity to subtract from the current Flat_buffer size.
@return the result of resizing. CCC_RESULT_OK if no errors occur or an error
indicating bad input has been provided.

If count would reduce the size to less than 0, the Flat_buffer size is set to 0
and the input error status is returned. */
CCC_Result CCC_flat_buffer_count_minus(CCC_Flat_buffer *buffer, size_t count);

/** @brief Set the Flat_buffer size to n.
@param[in] buffer the pointer to the buffer.
...
If count is larger than the capacity of the Flat_buffer the size is set equal to
the capacity and an error is returned. */
CCC_Result CCC_flat_buffer_count_set(CCC_Flat_buffer *buffer, size_t count);

/** @brief obtain the count of Flat_buffer active slots.
@param[in] buffer the pointer to the buffer.
...
[[nodiscard]] CCC_Count CCC_flat_buffer_count(CCC_Flat_buffer const *buffer);
...
is NULL. */
[[nodiscard]] CCC_Count CCC_flat_buffer_capacity(CCC_Flat_buffer const *buffer);
...
[[nodiscard]] CCC_Count
CCC_flat_buffer_sizeof_type(CCC_Flat_buffer const *buffer);

/** @brief Return the bytes in the Flat_buffer given the current count of active
elements.
@param[in] buffer the pointer to the buffer.
@return the number of bytes occupied by the current count of elements.

For total possible bytes that can be stored in the Flat_buffer see
CCC_flat_buffer_capacity_bytes. */
[[nodiscard]] CCC_Count
CCC_flat_buffer_count_bytes(CCC_Flat_buffer const *buffer);

/** @brief Return the bytes in the Flat_buffer given the current capacity
elements.
@param[in] buffer the pointer to the buffer.
@return the number of bytes occupied by the current capacity elements.

For total possible bytes that can be stored in the Flat_buffer given the current
element count see CCC_flat_buffer_count_bytes. */
[[nodiscard]] CCC_Count
CCC_flat_buffer_capacity_bytes(CCC_Flat_buffer const *buffer);

/** @brief return true if the size of the Flat_buffer is 0.
@param[in] buffer the pointer to the buffer.
@return true if the size is 0 false if not. Error if buffer is NULL. */
[[nodiscard]] CCC_Tribool
CCC_flat_buffer_is_empty(CCC_Flat_buffer const *buffer);

/** @brief return true if the size of the Flat_buffer equals capacity.
@param[in] buffer the pointer to the buffer.
@return true if the size equals the capacity. Error if buffer is NULL. */
[[nodiscard]] CCC_Tribool
CCC_flat_buffer_is_full(CCC_Flat_buffer const *buffer);
...
[[nodiscard]] void *CCC_flat_buffer_data(CCC_Flat_buffer const *buffer);
...
needed. Free the underlying Flat_buffer setting the capacity to 0. O(1) if no
...
buf. After all elements are processed the Flat_buffer is freed and capacity is
0. If destroy is NULL the Flat_buffer is freed directly and capacity is 0.
Elements are assumed to be contiguous from the 0th index to index at size - 1.*/
CCC_Result CCC_flat_buffer_clear_and_free(
    CCC_Flat_buffer *buffer,
...
Note that if destroy is non-NULL it will be called on each element in the
buf. However, the underlying Flat_buffer for the buffer is not freed. If the
...
CCC_Result CCC_flat_buffer_clear(
    CCC_Flat_buffer *buffer, CCC_Destructor const *destructor
);
...
#ifdef FLAT_BUFFER_USING_NAMESPACE_CCC
/* NOLINTBEGIN(readability-identifier-naming) */
typedef CCC_Flat_buffer Flat_buffer;
#    define flat_buffer_default(argument) CCC_flat_buffer_default(argument)
#    define flat_buffer_for(arguments...) CCC_flat_buffer_for(arguments)
#    define flat_buffer_with_storage(arguments...)                             \
        CCC_flat_buffer_with_storage(arguments)
#    define flat_buffer_with_allocator(arguments...)                           \
        CCC_flat_buffer_with_allocator(arguments)
#    define flat_buffer_with_capacity(arguments...)                            \
        CCC_flat_buffer_with_capacity(arguments)
#    define flat_buffer_from(arguments...) CCC_flat_buffer_from(arguments)
#    define flat_buffer_allocate(arguments...)                                 \
        CCC_flat_buffer_allocate(arguments)
#    define flat_buffer_reserve(arguments...) CCC_flat_buffer_reserve(arguments)
#    define flat_buffer_copy(arguments...) CCC_flat_buffer_copy(arguments)
#    define flat_buffer_clear(arguments...) CCC_flat_buffer_clear(arguments)
#    define flat_buffer_clear_and_free(arguments...)                           \
        CCC_flat_buffer_clear_and_free(arguments)
#    define flat_buffer_count(arguments...) CCC_flat_buffer_count(arguments)
#    define flat_buffer_count_bytes(arguments...)                              \
        CCC_flat_buffer_count_bytes(arguments)
#    define flat_buffer_count_plus(arguments...)                               \
        CCC_flat_buffer_count_plus(arguments)
#    define flat_buffer_count_minus(arguments...)                              \
        CCC_flat_buffer_count_minus(arguments)
#    define flat_buffer_count_set(arguments...)                                \
        CCC_flat_buffer_count_set(arguments)
#    define flat_buffer_capacity(arguments...)                                 \
        CCC_flat_buffer_capacity(arguments)
#    define flat_buffer_capacity_bytes(arguments...)                           \
        CCC_flat_buffer_capacity_bytes(arguments)
#    define flat_buffer_sizeof_type(arguments...)                              \
        CCC_flat_buffer_sizeof_type(arguments)
#    define flat_buffer_index(arguments...) CCC_flat_buffer_index(arguments)
#    define flat_buffer_is_full(arguments...) CCC_flat_buffer_is_full(arguments)
#    define flat_buffer_is_empty(arguments...)                                 \
        CCC_flat_buffer_is_empty(arguments)
#    define flat_buffer_at(arguments...) CCC_flat_buffer_at(arguments)
#    define flat_buffer_as(arguments...) CCC_flat_buffer_as(arguments)
#    define flat_buffer_back(arguments...) CCC_flat_buffer_back(arguments)
#    define flat_buffer_back_as(arguments...) CCC_flat_buffer_back_as(arguments)
#    define flat_buffer_front(arguments...) CCC_flat_buffer_front(arguments)
#    define flat_buffer_front_as(arguments...)                                 \
        CCC_flat_buffer_front_as(arguments)
#    define flat_buffer_allocate_back(arguments...)                            \
        CCC_flat_buffer_allocate_back(arguments)
#    define flat_buffer_emplace(arguments...) CCC_flat_buffer_emplace(arguments)
#    define flat_buffer_emplace_back(arguments...)                             \
        CCC_flat_buffer_emplace_back(arguments)
#    define flat_buffer_push_back(arguments...)                                \
        CCC_flat_buffer_push_back(arguments)
#    define flat_buffer_pop_back(arguments...)                                 \
        CCC_flat_buffer_pop_back(arguments)
#    define flat_buffer_pop_back_n(arguments...)                               \
        CCC_flat_buffer_pop_back_n(arguments)
#    define flat_buffer_move(arguments...) CCC_flat_buffer_move(arguments)
#    define flat_buffer_swap(arguments...) CCC_flat_buffer_swap(arguments)
#    define flat_buffer_write(arguments...) CCC_flat_buffer_write(arguments)
#    define flat_buffer_erase(arguments...) CCC_flat_buffer_erase(arguments)
#    define flat_buffer_insert(arguments...) CCC_flat_buffer_insert(arguments)
#    define flat_buffer_begin(arguments...) CCC_flat_buffer_begin(arguments)
#    define flat_buffer_next(arguments...) CCC_flat_buffer_next(arguments)
#    define flat_buffer_end(arguments...) CCC_flat_buffer_end(arguments)
#    define flat_buffer_reverse_begin(arguments...)                            \
        CCC_flat_buffer_reverse_begin(arguments)
#    define flat_buffer_reverse_next(arguments...)                             \
        CCC_flat_buffer_reverse_next(arguments)
#    define flat_buffer_reverse_end(arguments...)                              \
        CCC_flat_buffer_reverse_end(arguments)
#    define flat_buffer_data(arguments...) CCC_flat_buffer_data(arguments)
/* NOLINTEND(readability-identifier-naming) */
#endif /* FLAT_BUFFER_USING_NAMESPACE_CCC */

#endif /* CCC_FLAT_BUFFER_H */
